    ...
    <div class="col-sm">     
        ...
        <form v-on:submit.prevent="stopSubmit" v-if="singleItem" class="">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="mealTags" id="mealTags" v-model="editItem.tags" autocomplete="off" /> 
                <sub>Scheid de tags met een komma</sub>
            </div>
            ...
            <div class="form-group">
                ...
                <ul class="ingredientsCollection" v-if="editItem.ingredients">
                    <li v-for="(item, index) in editItem.ingredients" v-bind:key="item.uid">
                    <a  v-on:click="decrementAmountIngredient(index, $event)"> - </a>
                        {{ item.name }} 
                        <span v-if="editItem.ingredientsAmount[index] > 1">
                        x  {{editItem.ingredientsAmount[index]}} 
                        </span>
        ...
        <table class="items table" v-if="meals && meals.length > 0 && !singleItem">
            ...
            <tr v-for="item in meals" v-on:click="edit(item)" v-bind:key="item.uid">
                 <td>{{ item.name }}</td>
    ...
    methods: {
        ...
        registerNewIngredientAndAdd: async function()
        {
                ...
                { 
                    ...
                    this.$set(this.editItem.ingredientsAmount, this.editItem.ingredients.length, 0);
                }
              this.ingredientToAdd = null;
        },
        decrementAmountIngredient(i, ev)
        { 
            if (this.editItem.ingredientsAmount[i]> 1)
            {    
                this.$set(this.editItem.ingredientsAmount, i, this.editItem.ingredientsAmount[i]-1);
            }
            else 
            {
                this.editItem.ingredientsAmount.splice(i, 1);
        ...
        { 
            ...
            { 
                try 
                {
                    ...
                    if (response.data && this.filterValue(this.editItem.ingredients, "uid", response.data.uid) == null)
                    { 
